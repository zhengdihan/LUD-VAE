...
def test_mode(model, L, mode=0, refield=32, min_size=256, sf=1, modulo=1, sigma=None):
    '''
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    # Some testing modes
    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    if mode == 0:
        ...
        E = test_split_sigma(model, L, refield, min_size, sf, modulo, sigma)
    return E
...
def test_split_fn_sigma(model, L, refield=32, min_size=256, sf=1, modulo=1, sigma=None):
    '''
    model:
    L: input Low-quality image
    refield: effective receptive filed of the network, 32 is enough
    ...
    if h*w <= min_size**2:
        ...
        if h * w <= 4*(min_size**2):
            Es = [model(Ls[i], sigma) for i in range(4)]
        else:
            Es = [test_split_fn_sigma(model, Ls[i], refield=refield, min_size=min_size, sf=sf, modulo=modulo, sigma=sigma) for i in range(4)]
...
def test_split_sigma(model, L, refield=32, min_size=256, sf=1, modulo=1, sigma=None):
    E = test_split_fn_sigma(model, L, refield=refield, min_size=min_size, sf=sf, modulo=modulo, sigma=sigma)
    return E
