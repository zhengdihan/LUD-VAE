...
def kl_loss(mu, log_var, mu0=0, sigma=1, label=None):
    kl = 0.5*((1/(sigma**2))*(mu - mu0)**2 + torch.exp(log_var)/(sigma**2) + np.log(sigma**2) - log_var - 1)
    if label.sum() == 0:
        loss = (kl*label).mean(dim=(1,2,3)).sum()
    else:
        loss = (kl*label).mean(dim=(1,2,3)).sum() / label.sum()
