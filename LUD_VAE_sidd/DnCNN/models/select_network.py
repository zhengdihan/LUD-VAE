...
from utils import utils_model_dpir
...
def define_G(opt):
    ...
    # SDRUNet
    # ----------------------------------------
    elif net_type == 'sdrunet':
        from models_score.network_unet_score_sde import SUNetRes as net
        netG = net(in_nc=opt_net['in_nc'],
                   out_nc=opt_net['out_nc'],
                   nc=opt_net['nc'],
                   nb=opt_net['nb'],
    ...
    # GDnGFNet
    # ----------------------------------------
    elif net_type == 'gdngfnet':
        from models_score.network_neural_ode import GDnGFNet as net
        
        sigma_begin = opt_net['sigma_begin'] / 255.0
        sigma_end = opt_net['sigma_end'] / 255.0
        sigma_dist = opt_net['sigma_dist']
        num_classes = opt_net['num_classes']
        
        sigmas = utils_sample.get_sigmas(sigma_begin, sigma_end, num_classes, sigma_dist)
        
        n_channels = opt_net['in_nc']
        
        if n_channels == 3:
            model_name = 'sdrunet_color'
        elif n_channels == 1:
            model_name = 'sdrunet_gray'
        
        model_path = os.path.join('model_zoo', model_name+'.pth')
        from models_score.network_unet_score_sde import SUNetRes 
        scoremodel = SUNetRes(in_nc=n_channels+1, out_nc=n_channels, 
                              nc=[64, 128, 256, 512], nb=4, act_mode='R')
        scoremodel.load_state_dict(torch.load(model_path), strict=True)
        scoremodel.eval()
        for k, v in scoremodel.named_parameters():
            v.requires_grad = False
            
        score_device = torch.device("cuda:2")
        scoremodel = scoremodel.to(score_device)
        
        def compute_score(x_mod, sigma, denoise_net=scoremodel):
            net_device = x_mod.device
            
            x_input = x_mod.clone().to(score_device)
            sigma = sigma.clone().to(score_device)
            x_denoised = utils_model_dpir.test_mode(denoise_net, x_input, mode=6, 
                                                    refield=32, min_size=128, 
                                                    modulo=16, sigma=sigma)
            # x_denoised = denoise_net(x_input, sigma_input)
            score = (x_denoised - x_input) / sigma**2
            score = score.to(net_device)
            return score
