...
def test_mode(model, L, mode=0, refield=32, min_size=256, sf=1, modulo=1, sigma=None):
    ...
    if mode == 0:
        ...
        E = test_x8_sigma(model, L, modulo, sigma)
    elif mode == 7:
        E = test_onesplit_sigma(model, L, refield, min_size, sf, modulo, sigma)
...
def test_pad_sigma(model, L, modulo=16, sigma=None):
    h, w = L.size()[-2:]
    paddingBottom = int(np.ceil(h/modulo)*modulo-h)
    paddingRight = int(np.ceil(w/modulo)*modulo-w)
    L = torch.nn.ReplicationPad2d((0, paddingRight, 0, paddingBottom))(L)
...
def test_onesplit_sigma(model, L, refield=32, min_size=256, sf=1, modulo=1, sigma=None):
    '''
    model:
    L: input Low-quality image
    refield: effective receptive filed of the network, 32 is enough
...
def test_x8_sigma(model, L, modulo=1, sigma=None):
    E_list = [test_pad_sigma(model, util.augment_img_tensor(L, mode=i), modulo=modulo, sigma=sigma) for i in range(8)]
    for i in range(len(E_list)):
        if i == 3 or i == 5:
            E_list[i] = util.augment_img_tensor(E_list[i], mode=8 - i)
        else:
